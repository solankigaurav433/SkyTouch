...
const Parts = ({ navigation }) => {
  ...
  const data = [
    {
      id: 1,
      partno: '10x Magnification Glass',
      desc: '10x Magnification Glass',
      type: 'glass',
      category: 'GSE',
      serialized: 'no'
    },
    ...
    return (
      ...
      <View
        ...
        <View className="flex-row  items-start pb-1 space-x-2 my-1">
          <Fontisto name={'player-settings'} size={18} color={'black'} />
          <Text className="font-bold text-black text-[16px]">{item.category}</Text>
        ...
        <View className="flex-row justify-between space-x-1">
          <View className="bg-[#f2f2f2] px-1 py-[6px] rounded-[16px] my-1 w-[65%]">
            <Text className="font-bold text-black  text-[14px] flex-1 text-center">
              {item.desc}
            </Text>
          </View>
          <View className="bg-[#f2f2f2] p-1 rounded-[16px] my-1 w-[35%] items-center justify-center">
            <Text className="font-semibold items-center text-black text-[12px]  ">
              Serialized:{' '}
              {item.serialized === 'no' ? (
                <Icon1 name={'closecircleo'} size={12} color={'red'} />
              ) : (
        ...
        <View className="bg-[#f2f2f2] flex-row  px-2 py-[6px] justify-between items-center  rounded-[16px] my-1">
          <Text className="font-semibold text-black text-[13px]">Part No: {item.partno}</Text>
          <Text className="font-semibold text-black text-[13px] ">Type: {item.type}</Text>
        </View>
      </View>
    );
  };
